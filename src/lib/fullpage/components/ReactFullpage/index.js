...
// import fullpageStyles from '../../src/css/fullpage.css'; // eslint-disable-line no-unused-vars
import Fullpage from '../../src/js/fullpage'
...
class ReactFullpage extends React.Component {
  constructor(props) {
    ...
    // Fullpage = require('../../fullpage.extensions.min');
    ...
    propertiesThatNeedReRender.forEach((property) => {
      if (typeof prevProps[property] !== 'undefined') {
        if (Array.isArray(prevProps[property])) {
          isReRenderNecessary =
            isReRenderNecessary || !isEqualArray(prevProps[property], this.props[property])
        } else {
